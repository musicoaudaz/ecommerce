...
class User extends Model{
	...
	public static function login($login, $password){
		...
		if(count($results) === 0){
		    throw new \Exception("Usuário inexistente ou senha inválida."); //contra barra � pra achar a exception principal		
		}
		...
		if(password_verify($password, $data["despassword"]) === true){
			...
			//pegar din�micamente || cada resultado do banco vai criar uma atributo para cada campo
	...
	//m�todo que verifica se o usu�rio est� logado ou n�o
	public static function verifyLogin($inadmin = true){
		if(!User::checkLogin($inadmin)){ //se a sess�o n�o for definida
			if($inadmin){
	...
	public function save(){
		...
		$results = $sql->select("CALL sp_users_save(:desperson, :deslogin, :despassword, :desemail, :nrphone, :inadmin)", array( //chamada da procedure
			":desperson"=>urldecode($this->getdesperson()),
	...
	public static function getForgot($email, $inadmin = true){    
	    ...
	    if(count($results) === 0){
	        ...
	        if(count($results2) === 0){
	            ...
	            if($inadmin === true){
	                //link, endere�o que vai ser enviado o codigo e link do email parte administra��o
	                $link = "http://www.hcodecommerce.com.br/admin/forgot/reset?code=$code";
	            }else{
	                //link, endere�o que vai ser enviado o codigo e link do email parte dos clientes do site
